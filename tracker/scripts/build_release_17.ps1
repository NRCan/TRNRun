<#
.SYNOPSIS
    Copies Type3830 build files into the dist\TRNSYS17 folder structure for a GitHub release.
...
$dstSourceCode = Join-Path $RootPath "dist\TRNSYS17\SourceCode\Types\NRCan"

$srcBmp = Join-Path $RootPath "proformas\Type3830.bmp"
$dstProformas = Join-Path $RootPath "dist\TRNSYS17\Studio\Proformas\Utility (NRCan)\Progress Tracker"

$srcTmf = Join-Path $RootPath "proformas\Type3830_17.tmf"
# dstProformas reused, renamed to Type3830.tmf

$srcDebugDll = Join-Path $RootPath "build\DebugDLLs_17\Type3830.dll"
$dstDebugDll = Join-Path $RootPath "dist\TRNSYS17\UserLib\DebugDLLs"

$srcReleaseDll = Join-Path $RootPath "build\ReleaseDLLs_17\Type3830.dll"
$dstReleaseDll = Join-Path $RootPath "dist\TRNSYS17\UserLib\ReleaseDLLs"
